...
| **구분** | **내용** |
| --- | --- |
| **필요 권한** | Ceph 상태 조회는 관리자 또는 모니터링 권한이 있는 계정에서 수행합니다. |
| **외부 IT실체** | Ceph 정보는 Proxmox VE 및 Ceph 클러스터 상태를 기반으로 조회됩니다. |
| **사전 조건** | Proxmox API 연결, Ceph 서비스 구성, 노드 간 네트워크 연결이 정상이어야 합니다. |
| **데이터 기준** | 상태 정보는 10초 자동 갱신 또는 수동 새로고침 시점 기준으로 표시됩니다. |

## **3.5.1 Ceph Health 모니터링**
...
- **상태 진단 내역 (우측)** : 현재 발생한 구체적인 이슈 목록을 표시합니다. 영문 에러 코드를 **알기 쉬운 한글 명칭**으로 변환하여 제공합니다.
    ...
    - 정상 및 기타
        ...
        - **경고** : HEALTH_WARN 상태로 클러스터 작동에는 문제가 없으나, 주의가 필요한 상태입니다.
...
- **Monitors (감시자)** : 클러스터 맵(Cluster Map)과 쿼럼(Quorum) 상태를 관리합니다. Quorum에 참여하여 리더 또는 팔로워 역할을 수행 중입니다.
...
| **증상** | **확인 항목** | **조치 방법** |
| --- | --- | --- |
| Ceph 상태가 조회되지 않음 | Proxmox API 연결, Ceph 구성 여부 | API 연결을 확인하고 대상 클러스터에 Ceph가 구성되어 있는지 확인합니다. |
| HEALTH_WARN 표시 | 상태 진단 내역, OSD 용량, PG 상태 | 상세 모달에서 원본 메시지와 해결 가이드를 확인합니다. |
| HEALTH_ERR 표시 | OSD Down, PG Availability, Monitor Down | 즉시 상세 로그를 확인하고 장애 노드, 디스크, 네트워크 상태를 점검합니다. |
| 데이터 갱신 지연 | 자동 갱신 주기, 수동 새로고침 | 새로고침 버튼으로 재조회하고, 지연이 계속되면 Proxmox 작업 이력과 네트워크 상태를 확인합니다. |
