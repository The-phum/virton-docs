...
* **기본 관리자 계정** : 초기 아이디는 **admin**이며, 초기 비밀번호는 시험 또는 납품 시 별도 통보된 임시 비밀번호를 사용합니다.

> 제출용 문서에는 기본 관리자 비밀번호와 시험용 임시 비밀번호를 그대로 고정 기재하지 않는 것을 권장합니다. 시험 수행에 필요한 비밀번호는 별도 통보하고, 시험 종료 후 반드시 변경합니다.

## 입력 항목 및 허용 범위

| **항목** | **설명** | **입력 기준** |
| --- | --- | --- |
| **아이디** | VirtOn 사용자 계정 ID입니다. | 등록된 고유 계정 ID를 입력합니다. |
| **비밀번호** | 계정 인증에 사용하는 비밀값입니다. | 화면에는 마스킹되어 표시되며, 서버에 평문으로 저장되지 않습니다. |
| **접속 IP** | 로그인 요청을 수행한 관리자 PC의 IP입니다. | 화이트리스트가 설정된 경우 등록된 IP에서만 로그인이 허용됩니다. |
...
| **상황** | **화면 표시/동작** | **관리자 확인 항목** |
| --- | --- | --- |
| **로그아웃** | 로그인 화면으로 이동합니다. | 현재 브라우저에서 보호 기능 접근이 차단되는지 확인합니다. |
| **세션 만료** | 재로그인이 요구됩니다. | 세션 만료 후 기존 API 호출이 차단되는지 확인합니다. |
| **중복 로그인** | 기존 세션이 차단되거나 이후 로그인 세션 기준으로 동작합니다. | 감사 로그에서 로그인 및 세션 변경 이력을 확인합니다. |
...
| **상황** | **확인 방법** |
| --- | --- |
| **화이트리스트 미설정** | 일반 로그인 정책만 적용되는지 확인합니다. |
| **화이트리스트 설정** | 등록되지 않은 IP에서 로그인 화면 또는 인증 요청이 차단되는지 확인합니다. |
| **블랙리스트 차단** | 보안 알림과 감사 로그에서 차단 IP, 차단 시각, 해제 여부를 확인합니다. |

---

## **4.1.5 Proxmox API 설정**
...
| **증상** | **확인 항목** | **조치 방법** |
| --- | --- | --- |
| Proxmox 연결 실패 | Host, Port, Token ID, Secret 값 | Proxmox 콘솔에서 토큰 권한과 입력 형식을 다시 확인합니다. |
| 인증서 오류 | Proxmox 인증서 등록 여부 | 배포 절차의 인증서 등록 스크립트를 실행한 뒤 다시 시도합니다. |
| 설정 저장 불가 | 관리자 권한, 필수값 누락 | SUPER ADMIN 또는 ADMIN 계정으로 접속하고 모든 필수값을 입력합니다. |
