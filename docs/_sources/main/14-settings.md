...
| **구분** | **내용** |
| --- | --- |
| **필요 권한** | Proxmox API 연결, 계정/권한/Pool, 보안 정책, 시스템 백업, IP 접근 제어, SMTP 설정은 기능별로 ADMIN 또는 SUPER ADMIN 권한이 필요합니다. |
| **비밀정보 처리** | API Token Secret, 사용자 비밀번호, SMTP 비밀번호 등은 화면 입력 시 마스킹되며 제출문서에는 원문을 기재하지 않습니다. |
| **저장 보호** | 외부 연동 비밀값은 저장 시 암호화되며, 기존 비밀값의 평문은 조회되지 않습니다. 변경이 필요한 경우 새 값을 입력합니다. |
| **감사기록** | 계정, 권한, Pool, 보안 정책, IP 접근 제어, 백업/복구, SMTP 설정 변경은 감사 로그 확인 대상입니다. |

---

## 4.13.1 Proxmox API 연결 설정
...
| 토큰 ID       | `계정명@인증영역!토큰명` 형식                     | `root@pam!Test1` |
| 시크릿 키     | 해당 토큰의 시크릿 키 값 (암호화 저장)            | -                                   |

| 항목 | 입력 기준 |
| --- | --- |
| 호스트 | `http://` 또는 `https://`를 제외한 IP 주소 또는 도메인을 입력합니다. |
| 포트 | Proxmox API 기본 포트는 `8006`이며, 운영 환경에서 변경한 경우 해당 포트를 입력합니다. |
| 토큰 ID | `계정명@Realm!토큰명` 형식으로 입력합니다. |
| 시크릿 키 | Proxmox에서 발급한 Token Secret을 입력합니다. 저장 후 평문으로 다시 표시되지 않습니다. |
...
![API 설정 파트 메인 2](/_static/images/main/settings/setting_API2.png)
...
> 💡 **참고** 
> 환경에 따라 node.ip가 관리 API 접속 IP와 다를 수 있으니, 자동 등록 후 host IP를 꼭 확인하고 필요하면 수동 대기 API를 추가하거나 수정해주세요.
> 기본 API 저장 시 standby API는 /cluster/status 의 node.ip 기준으로 자동 등록됩니다.
...
## 4.13.3.1 역할(Role)별 기본 권한 리스트(기능 허용범위)
...
### 4.13.5.3.3 시스템 차단 해제 절차
> 시스템 차단 정책으로 전체 접근이 잠긴 경우, 사전에 발급한 **복구 시크릿(Recovery Secret)** 으로만 차단을 해제할 수 있습니다.

#### 1) 복구 시크릿 발급
![복구시크릿](/_static/images/main/settings/security/system_block1.png)
![복구시크릿2](/_static/images/main/settings/security/system_block2.png)

1. `설정 > 보안 관리 > 자체보호` 화면에서 **[복구 시크릿 발급]** 버튼을 클릭합니다.
2. 발급된 복구 시크릿은 **화면에 1회만 표시**되므로 즉시 확인하여 안전한 저장소(사내 비밀관리 시스템 등)에 보관합니다.
3. 복구 시크릿을 **재발급하면 이전 시크릿은 즉시 폐기**되어 사용할 수 없습니다.

#### 2) 시스템 차단 해제
![시스템 차단 해제](/_static/images/main/settings/security/system_block_recovery.png)

1. 시스템이 차단된 상태에서 `시스템 차단 해제` 화면으로 이동합니다.
2. 발급받아 보관한 복구 시크릿을 입력하고 **[차단 해제]** 를 실행합니다.
3. 정상 해제 시 로그인 및 주요 기능 접근이 다시 가능해집니다.
4. 해제 후에는 실패 원인(자체시험 실패 항목)을 반드시 점검한 뒤 운영을 재개합니다.
---

![버트온 백업 메인 페이지 1](/_static/images/main/settings/virton_backup_main.png)

## 4.13.5.4 시작하기 전에 (권한 및 접근)
...
## 4.13.5.4.2 백업 정책 설정 (Configuration)
...
## 4.13.5.4.6 문제 해결 (Troubleshooting)
...
## 4.13.5.5 관리자 IP 접근 제어 (IP Access Control)
...
### 4.13.5.5.2 주요 기능 사용법

#### 4.13.5.5.2.1 내 IP 간편 등록 (권장)
...
### 4.13.5.5.3 긴급 조치 가이드 (Lockout 상황)
...
> 이 기능은 시스템에 로그인한 사용자의 접속 유지 시간과 비활동 세션 관리 및 동시 접속 기기 수를 제어하여, 비인가자의 무단 접근이나 계정 공유로 인한 보안 사고를 방지하는 기능입니다.

- **세션 타임아웃:** 사용자가 로그인한 시점부터 계산하여, 설정한 시간이 경과하면 무조건 세션을 만료시키고 강제 로그아웃 처리하는 ‘최대 접속 허용 시간’을 설정합니다.
- **비활동 세션 종료 시간:** 사용자가 활동한 시점부터 계산하여, 설정한 시간이 경과하면 세션을 만료시키고 강제 로그아웃 처리하는 ‘비활동 허용 시간’을 설정합니다.
  1. 활동을 인정하는 항목은 클릭, 키 입력, 포인터, 라우트 이동, 저장 요청, 보호된 API 호출 (단, 폴링 및 자동은 제외)
  2. 비활동 세션 종료에 대한 불편함을 줄이기 위해 예외 역할 및 권한을 지정할 수 있습니다. 선택된 역할 및 선택된 해당 권한을 하나라도 가진 역할은 제외시킬 수 있습니다.
- **동시 접속 세션 수 제한:** 하나의 관리자 계정으로 동시에 여러 기기나 브라우저에서 로그인할 수 있는 최대 개수를 제한합니다.
- 세션 관련 설정은 새로운 세션 기반이므로 바로 적용을 원한다면 **로그아웃 후 다시 로그인** 해주세요.
...
## 4.13.6.2 내 알림 이메일 설정 (모든 사용자 공통)
...
## 4.13.6.3 전역 메일 발송 서버 (SMTP) 설정 (관리자 전용)
...
### 4.13.6.3.2 적용 및 테스트 순서

1. SMTP 서버 주소, 포트, 발신 계정, 인증정보를 입력합니다.
2. TLS/STARTTLS 사용 여부를 운영 메일 서버 설정과 동일하게 맞춥니다.
3. 테스트 메일을 발송하여 수신 여부를 확인합니다.
4. 저장 후 인증 실패, 계정 잠금, 시스템 경고 등 보안 알림이 이메일로 발송되는지 확인합니다.
...
## 4.13.7 설정 기능 문제 해결 방법

| **증상** | **확인 항목** | **조치 방법** |
| --- | --- | --- |
| Proxmox API 저장 실패 | Host, Port, Token ID, Secret, 관리자 권한 | 입력 형식과 토큰 권한을 확인하고 관리자 계정으로 다시 저장합니다. |
| 계정 생성 실패 | 계정 ID 중복, 비밀번호 정책, 역할 선택 | 중복 확인을 수행하고 비밀번호 정책을 모두 만족하는지 확인합니다. |
| 권한 변경이 적용되지 않음 | 역할 기본 권한, 개별 권한, Pool 범위 | 역할 권한과 Pool 할당을 함께 확인하고 재로그인 후 적용 상태를 확인합니다. |
| 자체시험 실패 | 실패 항목, 시스템 차단 상태, 감사 로그 | 자체시험 결과와 감사 로그를 확인하고 차단 해제 절차를 수행합니다. |
| 백업 실행 실패 | 백업 스토리지, 보존 기간, 진행 중 작업 | 스토리지 용량과 진행 중인 백업 작업을 확인합니다. |
| IP 접근 차단으로 로그인 불가 | 화이트리스트/블랙리스트, 긴급 조치 절차 | 허용 IP를 확인하고 필요 시 긴급 조치 가이드에 따라 복구합니다. |
| 이메일 발송 실패 | SMTP 서버, 포트, 인증정보, TLS 설정 | 테스트 메일을 다시 발송하고 메일 서버 인증 방식과 방화벽을 확인합니다. |
