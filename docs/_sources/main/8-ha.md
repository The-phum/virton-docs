...
| **구분** | **내용** |
| --- | --- |
| **필요 권한** | HA 리소스 추가, 수정, 제거와 Affinity Rules 설정은 관리자 권한이 필요합니다. |
| **사전 조건** | Proxmox 클러스터가 구성되어 있고 쿼럼이 정상이어야 합니다. HA 대상 VM/CT가 존재해야 합니다. |
| **영향 범위** | HA 정책은 장애 시 VM/CT 재시작, 재배치, 실행 위치에 직접 영향을 줍니다. 운영 서비스에 적용하기 전 대상 리소스와 노드를 확인합니다. |
| **감사/작업 이력** | HA 설정 변경과 리소스 이동은 감사 로그와 Proxmox 작업 이력 확인 대상입니다. |
...
### **4.7.2.8 HA 리소스 입력 항목 및 허용 범위**

| **항목** | **설명** | **입력/선택 기준** |
| --- | --- | --- |
| **ID** | HA로 관리할 VM/CT 리소스입니다. | 기존 VM 또는 CT 중 하나를 선택합니다. 수정 시 ID 변경은 불가합니다. |
| **상태** | HA 리소스의 목표 상태입니다. | `Started`, `Stopped`, `Ignored`, `Disabled` 중 운영 목적에 맞게 선택합니다. |
| **최대 재시작** | 동일 노드에서 재시작을 시도할 최대 횟수입니다. | 장애 반복 시 과도한 재시도를 방지할 수 있도록 운영 정책에 맞게 설정합니다. |
| **최대 재배치** | 다른 노드로 이동할 최대 횟수입니다. | 클러스터 노드 수와 서비스 중요도에 맞게 설정합니다. |
| **Failback** | 장애 복구 후 원래 노드로 되돌릴지 여부입니다. | 전용 노드가 필요한 서비스는 On, 안정성이 우선인 서비스는 Off를 검토합니다. |

---

## **4.7.3 HA 고급 설정 (Affinity Rules 설정)**
...
### **4.7.3.1 노드 친화성 규칙 (Node Affinity Rules)**
...
### **4.7.3.2 리소스 친화성 규칙 (Resource Affinity Rules)**
...
| **증상** | **확인 항목** | **조치 방법** |
| --- | --- | --- |
| HA 상태 조회 실패 | Proxmox API 연결, 클러스터 쿼럼 | [다시 시도]를 누르고 클러스터 페이지에서 쿼럼 상태를 확인합니다. |
| HA 리소스 추가 실패 | 대상 VM/CT, 권한, 리소스 중복 | 이미 HA에 등록된 리소스인지 확인하고 관리자 권한으로 다시 수행합니다. |
| 리소스가 기대 노드에서 실행되지 않음 | Node Affinity 규칙, Strict 여부, 노드 상태 | 가중치와 Strict 설정을 확인하고 대상 노드가 온라인인지 확인합니다. |
| 장애 후 재배치가 되지 않음 | 최대 재시작/재배치 값, 쿼럼, 스토리지 접근성 | HA 리소스 정책과 공유 스토리지 접근 가능 여부를 확인합니다. |
| 리소스 간 배치가 의도와 다름 | Resource Affinity 정책 | Positive/Negative 정책과 대상 리소스 그룹을 다시 확인합니다. |
