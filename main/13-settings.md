...
> 2. **관리자 계정**은 초기 **슈퍼관리자 계정**과 **동일한 역할의 권한**을 가지게됩니다.
...
- **자기 인식 (Auto-Discovery):**
  - 페이지 상단에 `자기 인식 완료: 현재 이 시스템은 Proxmox VMID [번호] 으로 매핑되어 있습니다.` 라는 문구가 표시됩니다.
  - 이는 VirtOn 서버가 스스로 Proxmox 내의 어떤 가상머신인지 식별했음을 의미하며, 이 번호를 기준으로 백업이 수행됩니다.
...
## 5.3 전역 메일 발송 서버 (SMTP) 설정 (관리자 전용)
