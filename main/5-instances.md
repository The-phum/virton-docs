...
| **구분** | **내용** |
| --- | --- |
| **필요 권한** | VM/CT 생성, 삭제, 전원 제어, 콘솔 접속, 리소스 변경은 사용자 역할과 Pool 할당 범위에 따라 허용 여부가 달라집니다. |
| **외부 IT실체** | 모든 제어 작업은 Proxmox VE API를 통해 수행되므로 API 연결이 정상이어야 합니다. |
| **사전 조건** | 대상 노드, 스토리지, 네트워크 브리지, ISO 또는 LXC 템플릿이 준비되어 있어야 합니다. |
| **감사기록** | VM/CT 생성, 삭제, 전원 제어, 콘솔 접속, 백업, 스냅샷 작업은 감사기록 또는 작업 이력 확인 대상입니다. |

## 3.3.1 가상 머신 & 컨테이너 목록 관리
...
| **항목** | **입력/선택 기준** |
| --- | --- |
| **노드** | 클러스터에 등록되어 있고 온라인 상태인 노드를 선택합니다. |
| **VM 이름** | 영문 대소문자, 숫자, 하이픈(-)만 사용합니다. |
| **CPU/메모리** | 대상 노드의 가용 자원을 초과하지 않도록 설정합니다. 메모리는 최소 512MiB 이상을 권장합니다. |
| **디스크** | 대상 스토리지의 여유 공간을 확인하고 용량을 지정합니다. |
| **네트워크** | 생성된 브리지 또는 VLAN 설정 중 VM에 연결할 네트워크를 선택합니다. |
| **ISO 이미지** | 사전에 업로드된 ISO 이미지를 선택합니다. 목록에 없으면 템플릿/업로드 메뉴에서 먼저 등록합니다. |

### 3.3.2.2 VM 설정 (부팅 순서 변경)
...
| **항목** | **입력/선택 기준** |
| --- | --- |
| **Hostname** | 영문 대소문자, 숫자, 하이픈(-)만 사용합니다. |
| **비밀번호/비밀번호 확인** | 두 값이 일치해야 하며, 운영 정책에 맞는 강한 비밀번호를 사용합니다. 비밀번호 원문은 제출문서에 기재하지 않습니다. |
| **OS 템플릿** | 스토리지에 등록된 LXC 템플릿을 선택합니다. 템플릿이 없으면 업로드 또는 다운로드 후 다시 생성합니다. |
| **CPU/메모리/Swap** | 대상 노드의 가용 자원을 기준으로 설정합니다. |
| **디스크/네트워크** | 스토리지, 마운트 포인트, 브리지, VLAN 등 운영 환경에 맞는 값을 선택합니다. |
...
### **3.3.3.2 리소스 사용량 및 트렌드 (Monitoring)**
...
| **증상** | **확인 항목** | **조치 방법** |
| --- | --- | --- |
| VM/CT 생성 실패 | 이름 형식, 필수값, 노드/스토리지 용량 | 입력값을 수정하고 대상 노드의 자원 여유를 확인합니다. |
| ISO 또는 템플릿이 보이지 않음 | 저장 데이터 유형, 업로드 위치 | 템플릿/업로드 메뉴에서 올바른 스토리지에 파일을 등록합니다. |
| 전원 제어 실패 | 현재 상태, 권한, Proxmox 작업 이력 | 상태를 새로고침하고 작업 이력에서 실패 사유를 확인합니다. |
| 콘솔 접속 실패 | VM/CT 실행 상태, 브라우저 설정, 권한 | 인스턴스가 실행 중인지 확인하고 콘솔 접근 권한을 점검합니다. |
| 백업/스냅샷 실패 | 스토리지 용량, 스냅샷 지원 여부, 작업 충돌 | 스토리지 기능과 진행 중인 작업을 확인한 뒤 다시 수행합니다. |
