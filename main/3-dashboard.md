...
| **구분** | **내용** |
| --- | --- |
| **필요 권한** | 로그인한 사용자는 권한 범위에 따라 조회 가능한 VM/CT와 작업 버튼이 달라질 수 있습니다. |
| **외부 IT실체** | 대시보드 정보는 Proxmox VE API에서 조회되므로 Proxmox API 연결 설정이 완료되어 있어야 합니다. |
| **데이터 기준** | 노드, 쿼럼, 스토리지, VM/CT 상태는 마지막 업데이트 시간 기준으로 표시됩니다. |
...
- 오류가 계속되면 [설정] > [Proxmox API 연결 설정]에서 활성 엔드포인트와 헬스체크 결과를 확인합니다.

---

## 3.1.3 가상 머신 및 컨테이너 목록
...
| **증상** | **확인 항목** | **조치 방법** |
| --- | --- | --- |
| 대시보드가 비어 있음 | Proxmox API 설정, 로그인 세션 | API 초기 설정을 완료하고 세션 만료 여부를 확인합니다. |
| 일부 VM/CT만 표시됨 | 사용자 권한, Pool 할당 범위 | 권한 관리와 Pool 설정에서 해당 계정의 접근 범위를 확인합니다. |
| 쿼럼 비정상 표시 | 클러스터 노드 상태, 네트워크 연결 | 클러스터 페이지에서 온라인 노드 수와 링크 상태를 확인합니다. |
| 스토리지 상태 오류 | 스토리지 활성 여부, 용량, 연결 상태 | 스토리지 페이지에서 대상 스토리지의 활성 상태와 사용률을 확인합니다. |
