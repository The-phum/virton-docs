...
| **구분** | **내용** |
| --- | --- |
| **필요 권한** | 스토리지 생성, 수정, 삭제는 관리자 권한이 필요한 관리 기능입니다. |
| **외부 IT실체** | NFS, SMB/CIFS, iSCSI는 외부 스토리지 서버와 연동되므로 서버 주소, 인증정보, export/share/target 설정이 사전에 준비되어야 합니다. |
| **영향 범위** | 스토리지는 VM/CT 디스크, ISO, 백업, 템플릿 저장 위치에 영향을 줍니다. 삭제 또는 비활성화 전 사용 중인 리소스를 확인합니다. |
| **비밀정보 처리** | SMB/CIFS 등 외부 스토리지 비밀번호는 화면 입력용이며 제출문서에는 원문을 기재하지 않습니다. |
...
스토리지 생성은 선택한 스토리지 유형에 따라 입력 항목이 달라집니다. 공통 필수값을 먼저 입력하고, 유형별 필수값을 추가로 입력합니다.

## **4.10.3.1 새 스토리지 생성 - 공통**
...
| **항목** | **입력/선택 기준** |
| --- | --- |
| **스토리지 이름** | 중복되지 않는 고유 이름을 입력합니다. 운영자가 식별 가능한 이름을 사용합니다. |
| **유형(Type)** | Directory, LVM, LVM-Thin, NFS, SMB/CIFS, iSCSI 중 운영 환경에 맞는 유형을 선택합니다. |
| **노드** | 전체 노드 또는 특정 노드만 선택합니다. 로컬 스토리지는 실제 존재하는 노드로 제한합니다. |
| **Content** | ISO, 백업, 디스크 이미지, 컨테이너 템플릿 등 저장할 데이터 유형을 선택합니다. |

## **4.10.3.2 새 스토리지 생성 - Directory**
...
- 삭제 전 해당 스토리지를 사용하는 VM/CT 디스크, ISO, 백업, 템플릿이 없는지 확인합니다.

## **4.10.6 문제 해결 방법**

| **증상** | **확인 항목** | **조치 방법** |
| --- | --- | --- |
| 스토리지 생성 실패 | 필수값, 이름 중복, 노드 선택 | 입력값을 확인하고 동일 이름 스토리지가 있는지 확인합니다. |
| Directory 생성 실패 | 경로 존재 여부, 쓰기 권한 | 대상 노드에 디렉터리가 존재하고 Proxmox가 접근 가능한지 확인합니다. |
| LVM/LVM-Thin 목록이 비어 있음 | 검색할 노드, VG/Thin Pool 존재 여부 | 노드를 먼저 선택하고 해당 노드에 볼륨 그룹 또는 씬 풀이 있는지 확인합니다. |
| NFS 연결 실패 | 서버 주소, export 경로, 방화벽 | NFS 서버 export 설정과 네트워크 접근 가능 여부를 확인합니다. |
| SMB/CIFS 연결 실패 | 계정/비밀번호, 공유 폴더, 도메인 | SMB 인증정보와 공유 이름을 확인합니다. 비밀번호 원문은 별도 관리합니다. |
| iSCSI Target 조회 실패 | Portal 주소, Target 서버 상태 | iSCSI 서버가 동작 중인지 확인하고 Portal 주소를 다시 입력합니다. |
| 삭제 실패 | 사용 중인 볼륨, 권한 | VM/CT, 백업, 템플릿이 해당 스토리지를 사용 중인지 확인합니다. |
